...
const EditItem = ({ item }: EditItemProps) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      showSuccessToast(t("items.editItemDialog.success"))
      reset()
      setIsOpen(false)
    },
    ...
    >
      ...
      <DialogContent>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>{t("items.editItemDialog.title")}</DialogTitle>
          </DialogHeader>
          <DialogBody>
            <Text mb={4}>{t("items.editItemDialog.description")}</Text>
            <VStack gap={4}>
              <Field
                required
              ...
              >
                <Input
                  {...register("title", {
                    required: t("items.addItemDialog.titleRequired"),
                  })}
          ...
          <DialogFooter gap={2}>
            <ButtonGroup>
              <DialogActionTrigger asChild>
                ...
                >
                  {t("items.editItemDialog.cancel")}
                </Button>
              </DialogActionTrigger>
              <Button variant="solid" type="submit" loading={isSubmitting}>
                {t("items.editItemDialog.submit")}
              </Button>
            </ButtonGroup>
          </DialogFooter>
        </form>
